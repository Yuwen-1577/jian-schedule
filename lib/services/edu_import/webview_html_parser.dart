...
class WebViewHtmlParser {
  ...
  static List<Course> parseHtml(String htmlStr) {
    ...
    for (var table in tables) {
      if (table.id == 'kbtable' || table.className.contains('table_border') || table.innerHtml.contains('星期') || table.innerHtml.contains('周')) {
      // 动态判断当前课表有几天（有些学校隐藏周日，只有6天）
      int totalDayCols = 7;
      final ths = table.querySelectorAll('th');
      int dayCount = 0;
      for (var th in ths) {
        if (th.text.contains('星期') || th.text.contains('周一') || th.text.contains('周二') || th.text.contains('周三') || th.text.contains('周四') || th.text.contains('周五') || th.text.contains('周六') || th.text.contains('周日')) {
          dayCount++;
        }
      }
      ...
      List<int> rowSpans = List.filled(totalDayCols, 0); // 记录 7 天的跨行情况
        ...
        for (int r = 1; r < rows.length; r++) { // 跳过表头
          final row = rows[r];
                  ...
                  if (lines.length >= 3) {
                    ...
                    for (int i = 1; i < lines.length; i++) {
                      if (lines[i].contains('节]')) {
                        ...
                        final weekMatch = RegExp(r'^(.*?)\[').firstMatch(timeStr);
                        final periodMatch = RegExp(r'\[(\d+)-(\d+)节\]').firstMatch(timeStr);
                        
                        if (periodMatch != null) {
                          final weeksStr = weekMatch != null ? weekMatch.group(1) ?? '' : '';
                          final weeks = CourseParserUtils.parseWeeks(weeksStr);
                          final startPeriod = int.tryParse(periodMatch.group(1) ?? '1') ?? 1;
                          final endPeriod = int.tryParse(periodMatch.group(2) ?? '2') ?? startPeriod;
                          ...
                          courses.add(Course(
                            ...
                            duration: duration,
                            activeWeeks: weeks.isEmpty ? List.generate(20, (i) => i+1) : weeks,
                            colorValue: presetColors[random.nextInt(presetColors.length)],
                          ));
                        }
                      }
                    }
  ...
  static String _extractTextWithNewlines(Node node) {
    if (node is Text) {
      ...
      String content = node.nodes.map((n) => _extractTextWithNewlines(n)).join('');
      if (content.contains('----------')) {
